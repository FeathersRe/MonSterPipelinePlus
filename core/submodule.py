...
class BasicConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_bn:
            x = self.bn(x)
        if self.relu:
            x = nn.LeakyReLU(inplace=False)(x)#, inplace=False)
        return x
...
class BasicConv_IN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_in:
            x = self.IN(x)
        if self.relu:
            x = nn.LeakyReLU(inplace=False)(x)
        return x
...
def build_gwc_volume(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        ...
        pad = (i, 0)
        corr_padded = F.pad(corr, pad=pad, mode='constant', value=0)
        volume_list.append(corr_padded.unsqueeze(2))
...
def build_norm_correlation_volume(refimg_fea, targetimg_fea, maxdisp):
    B, C, H, W = refimg_fea.shape
    volume = torch.zeros((B, 1, maxdisp, H, W), 
                     device=refimg_fea.device, 
                     dtype=refimg_fea.dtype)
    for i in range(maxdisp):
        if i > 0:
...
def build_concat_volume(refimg_fea, targetimg_fea, maxdisp):
    B, C, H, W = refimg_fea.shape
    volume = torch.zeros((B, 2 * C, maxdisp, H, W), 
                     device=refimg_fea.device, 
                     dtype=refimg_fea.dtype)
    for i in range(maxdisp):
        if i > 0:
...
def context_upsample(disp_low, up_weights):
    ...
    # Assumes disp_low is [b, 1, h, w] (single-channel disparity)
# Add padding first
    disp_padded = F.pad(disp_low, (1, 1, 1, 1), mode='replicate')  # [b, 1, h+2, w+2]

    # Create 9 shifted versions (3x3 patch)
    neighbors = []
    for dy in [-1, 0, 1]:
        for dx in [-1, 0, 1]:
            shifted = disp_padded[:, :, 1+dy:h+1+dy, 1+dx:w+1+dx]  # crop to original size
            neighbors.append(shifted)

    # Stack into [b, 9, h, w]
    disp_unfold = torch.cat(neighbors, dim=1)  # [b, 9, h, w]
